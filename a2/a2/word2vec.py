...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = U.T @ (y_hat - y)

    gradOutsideVecs = (y_hat - y).reshape(-1, 1) @ v_c.reshape(1, -1)
...
def negSamplingLossAndGradient(
    ...
    gradCenterVec = u_o * (sigmoid(u_o@v_c) - 1) - \
        np.sum(U_k.T * (sigmoid(-U_k@v_c) - 1), axis=1)

    temp = (1 - sigmoid(-U_k @ v_c)).reshape(-1, 1) @ v_c.reshape(1, -1)

    gradOutsideVecs = np.zeros(U.shape)
    for i in range(len(negSampleWordIndices)):
        gradOutsideVecs[negSampleWordIndices[i], :] += temp[i, :]
    gradOutsideVecs[outsideWordIdx] = (sigmoid(u_o @ v_c) - 1) * v_c
    # END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
            ...
            temp_loss, temp_gradCenterVecs, temp_gradOutsideVectors = word2vecLossAndGradient(
                centerWordVec,
                outsideWordIdx,
                outsideVectors,
                dataset
            ...
            loss += temp_loss
            gradCenterVecs[currentCenterWordIdx] += temp_gradCenterVecs
