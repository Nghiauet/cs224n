...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    att = (query @ key.transpose(-2,-1)) * (1.0 / np.sqrt(self.attention_head_size)) # B,nh,T,T
    B , nh, T,T = att.size()
    att = att + attention_mask

    att = F.softmax(att, dim = -1)
    att = self.dropout(att)
    att = (att @ value).transpose(1,2)  # B ,nh, T, T @ B, nh, T, hs -> B, nh, T, hs -> B, T, nh,hs
    att = att.contiguous().view(B,T,self.all_head_size)
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    input = dropout(dense_layer(input))
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    h_p = self.add_norm(att, hidden_states, self.attention_dense, self.attention_dropout, self.attention_layer_norm)

    intermidiate_dense  = self.interm_af(self.interm_dense(h_p))

    BL = self.add_norm(intermidiate_dense,h_p , self.out_dense, self.out_dropout, self.out_layer_norm)
    return BL
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    output = self.embed_dropout(self.embed_layer_norm(pos_embeds + inputs_embeds + tk_type_embeds))
    return output
