...
def minibatch_parse(sentences, model, batch_size):
    ...
    while not empty:
        ...
        empty = True
        for i in range(n):
            element = unfinished_parses[i]
            if len(element.stack) > 1 or len(element.buffer)>0:
        ...
        for i in range(len(y_hat)):
            batch[i].parse_step(y_hat[i])
            # print("batch              : ",i)
            # print("stack of batch     :", batch[i].stack)
            # print("buffer of batch    :",batch[i].buffer)
            # print("dependency of batch:", batch[i].dependencies)
            # print("sentence of batch  : ", batch[i].sentence)
            # print("---------------------------")
            # print()
    dependencies = [paser.dependencies for paser in unfinished_parses]
    ### END YOUR CODE
